...
- **Acción:** Publica con estrategias en cascada para no depender del locale, del tamaño de la ventana ni del DOM: ajusta el viewport al tamaño real de la pantalla (LinkedIn decide los controles según el viewport de la página, no la ventana física; así la ventana nunca queda recortada), (1) va a /feed/ y hace clic en el disparador del post (etiquetas multi-locale + selectores por clase estables; flujo que funcionó en ejecución real) y descarta modales/upsells in-page (Premium/Plus, cookies) con Escape o botones de cierre; (2) si no abre, recurre a la URL directa del editor (`/post/new/`, sin depender del botón "Start a post"); (3) rellena el editor contenteditable y publica por etiqueta ("Publicar"/"Post"/"Publish"/"Publier"), o con el botón primario (clase estable, en modal o a página completa), o con el atajo de teclado Ctrl/Cmd+Enter como último recurso. Confirma el éxito cuando el composer se cierra o aparece el toast de confirmación. Si todo falla, devuelve un error con lo que se intentó y cómo reintentar.
